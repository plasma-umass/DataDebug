...
Throughout this section, we illustrate these algorithms with a running
...
management system, data inputs would be tuples (records) and
computations would be queries.
...
significantly alters the average will thus be considered to have a
large impact on the graph. Any similar summary statistic like standard
deviation would also be suitable.
...
average absolute deviation for each data item's
impact. Section~\ref{sec:asymptotic_analysis} shows that the runtime
efficiency of this approach is asymptotically optimal, and
Section~\ref{sec:sampling_effectiveness} explains why it is nearly as
