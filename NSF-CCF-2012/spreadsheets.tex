\section{Small-Scale Data Debugging: Spreadsheets}
...
Data debugging operates in several phases: (1) computing the dependence
graph, (2) performing impact analysis, and then (3) ranking impacts. Runtime
...
structure is atypical. Dependence graphs normally form a polytree or a
forest of polytrees. In this case, the cost of constructing the dependence
...
To empirically measure the runtime of \checkcell{}, we run it on a
random subset of 30 spreadsheets drawn from the EUSES
corpus~\cite{Fisher:2005:ESC:1082983.1083242}, excluding those that do
not contain formulas. Figure~\ref{fig:execution_time_graph} reports
the performance of data debugging across our spreadsheet suite.
...
cost of the impact analysis.

The spreadsheets that require the most execution time both have by far
the largest number of formulas, and the latter also has the largest
number of data. Their relatively high execution time is attributable
to the fact that the cost of impact analysis increases as the number
of formulas increases, since the Excel recalculation engine must do
more work per item tested. The \texttt{intresults} spreadsheet also
has an extremely highly-connected clique in its dependence graph,
which makes the dependence analysis and impact analysis phases more
expensive.
...
\emph{sign omission}, where a negative sign was dropped; \emph{magnitude errors}, a change in a value (usually a dropped or spurious digit) that results in an order of magnitude increase or decrease;
\emph{digit transposition}, where at least two digits are transposed; and
\emph{typos}, any other typographical error (e.g., a mistyped digit).
...
(\frac{10}{9})(\sum_{j=1}^{10}{A_j} - A_i)$.  By storing the result of
the first sum, it is possible to efficiently compute the impact of
each $A_i$ directly (without sampling).


\begin{figure*}[!t]
\centering
\includegraphics[width=5.5in]{images/frequency-formulas}
  \caption{The frequency of formulas over ranges, measured over a random sample of 200 spreadsheets drawn from the EUSES repository. Well-behaved functions like SUM, MIN, and AVERAGE dominate, demonstrating ample opportunity for our proposed optimizations.\label{fig:frequency_formulas}}
\end{figure*}

To evaluate the opportunity for this optimization, we wrote a tool to
count the occurrences of functions over ranges from a random selection
of spreadsheets drawn from the EUSES repository. Figure~\ref{fig:frequency_formulas}
presents the results, obtained from 200 spreadsheets. As the figure
shows, uses of the sum function dominate all other functions.
The next most frequent functions are min and average, both of which
are amenable to our proposed optimization.

When a computation consists entirely of these
...
like addition.
We plan to handle dependent chains of functions by
...
% performance by caching parts of functions that do not depend on 

% \comment{Another technique: use of intermediate results. Break the function into components. Some of them do not change in every iteration, so you can precompute part of the function. This is slightly different from the previous point.}
% EDB: Hm. will have to think about this.
